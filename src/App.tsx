...
interface DubEntry {
  ...
  media?: {
    media: {
      ...
      episodes: number;
      averageScore: number;
      airingSchedule: { nodes: any[] };
    };
  };
}

interface ScheduleEntry extends DubEntry {
  displayTime: string;
  timeOnly: string;
  dayOfWeek: string;
}
...
const fetchDubSchedule = async (): Promise<DubEntry[]> => {
  try {
    const res = await fetch(DUB_SCHEDULE_URL);
    if (!res.ok) throw new Error("Failed to fetch dub schedule");
    return await res.json();
  } catch (error) {
    console.error("Error fetching dub schedule:", error);
    return [];
  }
};

const formatAirTime = (dateString: string) => {
...
const DubScheduleView = ({ onSelectMedia }: DubScheduleViewProps) => {
  ...
  useEffect(() => {
    const loadSchedule = async () => {
      setLoading(true);
      const schedule = await fetchDubSchedule();
...
const AnimeRow = ({ title, animes, onSelect }: AnimeRowProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="fixed left-0 top-0 w-[5px] h-full z-[450] group/trigger">
        <div
          className={`fixed z-[400] top-1/2 -translate-y-1/2 w-20 h-[750px] bg-white/5 backdrop-blur-3xl border border-white/10 rounded-full flex flex-col items-center py-12 space-y-10 shadow-2xl transition-all duration-500 ${
            isFullscreenApp
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth relative">
          {/* DUB RELEASES VIEW */}
          {view === "dub-releases" && (
            <DubScheduleView onSelectMedia={onSelectMedia} />
          )}
...
// FULL CALENDAR VIEW
const FullCalendarView = ({ dubSchedule, onSelectMedia, onBack }: any) => {
  const [currentMonth, setCurrentMonth] = useState(new Date());
  ...
  const getEntriesForDay = (day: number) => {
    ...
    return dubSchedule.filter((entry) =>
      isSameDay(new Date(entry.episodeDate), date)
    );
  };
...
// RECTANGULAR WEEK OVERVIEW CARD
const WeekOverviewCard = ({ entry, onSelect }: any) => {
  const media = entry.media?.media;
  if (!media) return null;
...
// DUB SCHEDULE VIEW
const DubScheduleView = ({ onSelectMedia }: any) => {
  const [dubSchedule, setDubSchedule] = useState<ScheduleEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");
  const [viewMode, setViewMode] = useState("weekly"); // 'weekly' or 'calendar'
